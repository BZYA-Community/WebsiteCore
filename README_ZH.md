...
- **可插拔特性**：存储（LocalOSS/阿里OSS）、搜索（Meilisearch 或 SQL 直查回退）、数据库（PostgreSQL）等均通过 `Features` 开关装配
...
数据库仅支持 PostgreSQL；全文搜索使用 Meilisearch（可选），未开启时自动回退到 PostgreSQL `ILIKE` 模糊匹配。
...
新增 API 的标准流程：在 `mirc/web/v1/` 声明接口签名 → `make gen-mir` 生成路由骨架 → 在 `internal/servants/web/` 实现业务。数据库结构变更在 `scripts/migration/postgres/` 按编号新增 `NNNN_name.{up,down}.sql`。完整开发指南见 [docs/development.md](docs/development.md)。
