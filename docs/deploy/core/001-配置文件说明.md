...
Features:
  ...
  Demo: ["Base", "MySQL", "Option", "Sms", "LocalOSS", "LoggerFile"]
...
Web、Docs 等 HTTP 子服务仍可共享端口，将对应的 HttpIp / HttpPort 配置为相同值即可。
管理功能由 Web API 提供，LocalOSS 下载由 Web 上的 `/oss/` 路由提供。

```yaml
Features:
  Default: ["Postgres", "Meili", "LocalOSS", "BigCacheIndex", "LoggerFile", "Web", "Docs"]
  Docs: ["Docs:OpenAPI"]
WebServer:
  HttpIp: 0.0.0.0
  HttpPort: 8010
