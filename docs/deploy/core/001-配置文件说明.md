> 数据库仅支持 PostgreSQL，数据库访问统一使用 GORM；二者始终启用，无需添加数据库或 ORM 特性。连接信息在 `Postgres` 段配置，连接池参数在 `Database` 段配置。
...
Features:
  Default: ["Base", "Option", "LocalOSS", "LoggerFile"]
  Develop: ["Base", "Option", "Sms", "AliOSS", "LoggerZinc"]
  Demo: ["Base", "Option", "Sms", "MinIO", "LoggerZinc"]
...
这里 `Default`套件 代表的意思是： 使用`Base/Option` 中的功能，外加 `LocalOSS/LoggerFile`功能，也就是说开启了`Zinc/Redis/Alipay/SimpleCacheIndex/LocalOSS/LoggerFile` 6项功能；
`Develop`套件依例类推。 
...
# 手动指定需要开启的功能集
release/paopao-ce --no-default-features --features localoss,loggerfile,redis
```
...
Features:
  Default: ["Zinc", "LocalOSS",  "BigCacheIndex", "Web"]
WebServer: # Web服务
  HttpIp: 0.0.0.0
  HttpPort: 8010
  ReadTimeout: 60
...
Features:
  Default: ["Zinc", "LocalOSS",  "BigCacheIndex", "Web", "Admin", "Bot"]
WebServer: # Web服务
  HttpIp: 0.0.0.0
  HttpPort: 8010
  ReadTimeout: 60
